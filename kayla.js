...
try {
        ...
        const UltahCreator = new Date('3 Desember, 2022 00:00:00')
        const CreatorZykuan = new Date().getTime()
        const Xezy = UltahCreator - CreatorZykuan
        const kayhari = Math.floor( Xezy / (1000 * 60 * 60 * 24));
        const kayjam = Math.floor( Xezy % (1000 * 60 * 60 * 24) / (1000 * 60 * 60))
        const kaymenit = Math.floor( Xezy % (1000 * 60 * 60) / (1000 * 60))
        const kaydetik = Math.floor( Xezy % (1000 * 60) / 1000)
...
if (isCmd) {
console.log(chalk.yellow.bgCyan.bold(' - 𝙓𝙚𝙯𝙮 '), color(`[ PESAN ]`, `${halalu}`), color(`FROM`, `${halalo}`), color(`${pushname}`, `${halali}`), color(`Text :`, `${halala}`), color(`${body}`, `${halale}`))
}
...
kayla.sendMessage(m.chat,{text:`Mau Script Bot Nya Ya? Awokawok ${quoted:m})
}
break
case 'q': case 'quoted': {
